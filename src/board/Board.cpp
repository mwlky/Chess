...
namespace Chess {
    ...
    void Board::ReleasePiece() {
        ...
        bool checkAfterMove =
                isValid && SimulateMoveAndCheckForCheck(m_DraggedPawn, newX, newY);
    ...
    bool Board::IsTryingToCastle(int newX, int newY) const {
        ...
        if (IsPathUnderAttack(m_DraggedPawn->GetBoardXPosition(), newX, newY,
                              m_DraggedPawn->GetSite()))
            return false;
    ...
    bool Board::CheckIfMoveIsProper(int newX, int newY,
        ...
        if (pawnOnTargetSquare != nullptr &&
            ...
            isValid = piece->IsValidMove(newX, newY, moveType) &&
                      CheckIfPathIsClear(*piece, newX, newY);
    ...
    bool Board::IsPathUnderAttack(int xStart, int xEnd, int y,
        ...
        for (int x = xStart; x != xEnd + directionNormalized;
             x += directionNormalized) {
    ...
    bool Board::HasTriedToSaveKing(int newX, int newY) {
        ...
        if (king != nullptr) {
            ...
            if (pawnOnTargetSquare != nullptr &&
                pawnOnTargetSquare->GetSite() == m_CurrentMove)
                return false;
    ...
    bool Board::SimulateMoveAndCheckForCheck(std::shared_ptr<Piece> piece,
        ...
        auto king = FindKingOfSite(m_CurrentMove);
        bool isCheck = IsSquareUnderAttack(king->GetBoardXPosition(),
                                           king->GetBoardYPosition(), m_CurrentMove);
    ...
    bool Board::CheckCheckmate(Piece::Site site) {
        ...
        if (!IsSquareUnderAttack(king->GetBoardXPosition(), king->GetBoardYPosition(),
                                 site))
            return false;
    ...
    Piece::MoveType Board::GetMoveType(int newX, int newY,
        ...
        Piece::MoveType moveType =
                (m_Squares.squares[newX][newY].GetAssignedPawn() != nullptr)
                ? Piece::MoveType::TAKE
                : Piece::MoveType::NORMAL;

        moveType =
                IsEnPassant(newX, newY, piece) ? Piece::MoveType::EN_PASSANT : moveType;

        return moveType;
    }

    bool Board::IsEnPassant(int newX, int newY,
                            const std::shared_ptr<Piece> &piece) {

        auto movedPawn = dynamic_cast<Pawn *>(piece.get());
        ...
        auto pieceOnSquare = m_Squares.squares[newX][std::min(7, std::max(0, newY + direction))].GetAssignedPawn();
        if (pieceOnSquare == nullptr)
            return false;

        auto pawnToTake = dynamic_cast<Pawn *>(pieceOnSquare
                .get());
        ...
        if (pawnToTake != nullptr)
            if (pawnToTake->EnPassantPossible)
                return true;
    ...
    void Board::CheckForPromotion() {
        for (auto &square: m_Squares.squares) {
            ...
            auto piece = square[m_CurrentMove == Piece::Site::WHITE ? 0 : 7].GetAssignedPawn();
            if (piece == nullptr)
                continue;
            ...
            if (pawn != nullptr) {
                m_PawnToPromote = pawn;
                break;
            }

            m_PawnToPromote = nullptr;
        }
    }
    ...
    void Board::PromotionLogic() {
        if (m_PawnToPromote == nullptr)
            return;

        int pawnX = m_PawnToPromote->GetBoardXPosition();
        int pawnY = m_PawnToPromote->GetBoardYPosition();

        auto site = m_CurrentMove == Piece::Site::WHITE ? QUEEN_WHITE : QUEEN_BLACK;

        auto blackQueen =
                std::make_shared<Queen>(site, m_CurrentMove, pawnX, pawnY);

        m_Squares.squares[pawnX][pawnY].UnassignPiece();
        m_Squares.squares[pawnX][pawnY].AssignPiece(blackQueen, false);
        m_PawnToPromote = nullptr;
        SwitchSite();
    }
} // namespace Chess